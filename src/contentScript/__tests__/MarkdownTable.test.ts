...
describe('MarkdownTable', () => {
    ...
    it('does not delete the final column', () => {
        ...
        const unchanged = table.deleteColumn(0);

        expect(unchanged).toBe(table);
        expect(unchanged.headerCells).toEqual(['H1']);
        expect(unchanged.alignments).toEqual(['left']);
        expect(unchanged.bodyRows).toEqual([['A1'], ['B1']]);
    });
...
describe('serializeWithOffsets agrees with parsing the serialization', () => {
    const cases: Record<string, () => MarkdownTable> = {
        ...
        'empty and adjacent-empty cells': () =>
            MarkdownTable.fromParts({
                headerCells: ['', 'H2', ''],
        ...
        'br tags the serializer normalizes': () =>
            MarkdownTable.fromParts({
                headerCells: ['H'],
                alignments: [null],
                bodyRows: [['one<BR/>two'], ['three<br />four']],
            }),
        'ragged input padded to width': () =>
            MarkdownTable.fromParts({
                headerCells: ['H1'],
                alignments: [],
                bodyRows: [['a', 'b', 'c'], ['d']],
        ...
        'header only': () =>
            MarkdownTable.fromParts({ headerCells: ['H1', 'H2'], alignments: [null, null], bodyRows: [] }),
        'wide cell content': () =>
            MarkdownTable.fromParts({
