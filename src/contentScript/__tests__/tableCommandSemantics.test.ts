...
describe('tableCommandSemantics', () => {
    ...
    describe('insertRowForActiveCell', () => {
        it('should insert row BEFORE header (new header created)', () => {
            ...
            expect(result.headerCells).toEqual(['', '']); // New empty header
            expect(result.bodyRows[0]).toEqual(['H1', 'H2']); // Old header is now first body row
            expect(result.bodyRows[1]).toEqual(['R1C1', 'R1C2']);
            expect(result.bodyRows.length).toBe(3);
        });
        ...
        it('should insert row AFTER header (new first body row)', () => {
            ...
            expect(result.headerCells).toEqual(['H1', 'H2']);
            expect(result.bodyRows[0]).toEqual(['', '']); // New empty row
            expect(result.bodyRows[1]).toEqual(['R1C1', 'R1C2']);
            expect(result.bodyRows.length).toBe(3);
        });
        ...
        it('should insert row BEFORE body row', () => {
            ...
            expect(result.bodyRows[0]).toEqual(['R1C1', 'R1C2']);
            expect(result.bodyRows[1]).toEqual(['', '']);
            expect(result.bodyRows[2]).toEqual(['R2C1', 'R2C2']);
        });
        ...
        it('should insert row AFTER body row', () => {
            ...
            expect(result.bodyRows[1]).toEqual(['R2C1', 'R2C2']);
            expect(result.bodyRows[2]).toEqual(['', '']);
        });
    });
    ...
    describe('deleteRowForActiveCell', () => {
        it('should delete header (promote first row)', () => {
            ...
            expect(result.headerCells).toEqual(['R1C1', 'R1C2']);
            expect(result.bodyRows.length).toBe(1);
            expect(result.bodyRows[0]).toEqual(['R2C1', 'R2C2']);
        });
    ...
    describe('moveRowForActiveCell', () => {
        ...
        it('should swap header with first body row when moving header DOWN', () => {
            ...
            expect(result.headerCells).toEqual(['R1C1', 'R1C2']);
            expect(result.bodyRows[0]).toEqual(['H1', 'H2']);
        });
    ...
    describe('moveColumnForActiveCell', () => {
        it('should swap columns', () => {
            ...
            expect(result.headerCells).toEqual(['H2', 'H1']);
            expect(result.bodyRows[0]).toEqual(['R1C2', 'R1C1']);
        });
    });
});
